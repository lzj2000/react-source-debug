...
function completeUnitOfWork(wip) {
  do {
    ...
    // 合并子树的 flags 到父节点，便于提交阶段统一遍历
    bubbleProperties(wip);
    // 转向兄弟；若没有兄弟则继续回溯到父
    const sibling = wip.sibling;
...
// 渲染完成后：HostText 的 completeWork 发现文本变化 ⇒ 标记 Update
// 提交阶段：commitHostTextUpdate(stateNode, 'hello', 'world') 实际更新真实 DOM 文本
```
