...
高执行频率：MessageChannel 的 postMessage 会触发宏任务，执行时机在每帧渲染后，比 requestIdleCallback 更频繁，能保证任务及时执行。
全浏览器兼容：支持 IE10+ 及所有现代浏览器，满足 React 的兼容性要求。
可控性强：可手动控制消息的发送和接收，实现任务的立即调度或暂停，还能为任务设置优先级。
