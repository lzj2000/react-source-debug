...
在 React 18 中，批量更新不再依赖于一个简单的全局 `isBatchingUpdates` 标志。相反，它通过更复杂的调度机制实现。当更新被触发时，它们会被加入到一个更新队列中，并由调度器在合适的时机（通常是在浏览器下一帧绘制之前）统一处理。

例如，在事件处理函数中，React 会在事件处理结束时，统一刷新所有挂起的更新。这类似于一个隐式的 `batchedUpdates` 包装器，但开发者无需手动干预。
...
#### 3. `updateReducer` 中的更新处理

在 `updateReducer`（或 `updateState` 等 Hook 内部更新逻辑）中，更新会被添加到 Hook 对应的 `updateQueue` 中。调度器会根据优先级和时间切片机制，在合适的时机从 `updateQueue` 中取出更新并计算新的状态。

在 React 中，更新的调度由 `Scheduler` 模块统一管理，它会根据任务的优先级和剩余时间来决定何时执行更新，从而实现更高效的并发渲染。
...
React 18 通过默认的自动批量更新机制，极大地简化了 `setState` 的行为，使其在大多数情况下都是“异步”的（即批量处理）。只有在明确需要同步更新的场景下，才需要使用 `ReactDOM.flushSync`。
