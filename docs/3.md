...
2. 事件优先级与包装：React DOM 事件系统会根据事件类型选择优先级（离散/连续/默认），并在分发时设置当前更新优先级，比如离散事件（如 `click`、`keydown`、`input` 等）会提升到 `DiscreteEventPriority`。现代并发模式下默认已批量，但还保留`unstable_batchedUpdates` 用于 legacy 兼容，。
...
```javascript
/**********************
 * A. 事件层包装（如示例中的handleClick，设置事件优先级）
 **********************/
...
export function discreteUpdates(fn, a, b, c, d) {
  // 离散事件提升当前更新优先级到 DiscreteEventPriority
  return discreteUpdatesImpl(fn, a, b, c, d);
}

/**********************
 * B. Hook 的 setState 入队与调度（react-reconciler/ReactFiberHooks）
 **********************/
function dispatchSetState(fiber, queue, action) {
  ...
  // 急切状态优化：当队列当前为空时，尝试提前用 reducer 计算新状态
  if (
    fiber.lanes === NoLanes &&
  ...
  // 并发 Hook 更新入队，并合并到根的调度中
  const root = enqueueConcurrentHookUpdate(fiber, queue, update, lane);
  if (root !== null) {
    scheduleUpdateOnFiber(root, fiber, lane);
    entangleTransitionUpdate(root, queue, lane); // 关联过渡（如 startTransition）
  }
}

/**********************
 * C. scheduleUpdateOnFiber（签名与决策点）
 * - 合并到根的调度，并在必要时对 legacy 根进行立即同步刷新
 **********************/
export function scheduleUpdateOnFiber(root, fiber, lane) {
  ...
  // 仅在 legacy 根 + SyncLane + 非并发 fiber + 不在 render/commit 上下文时，立即同步刷新
  if (
    lane === SyncLane &&
...
function updateStateInRender(hook) {
  ...
  if (pending !== null) {
    ...
    do {
      ...
      newState = queue.lastRenderedReducer
        ? queue.lastRenderedReducer(newState, action)
...
/**********************
 * E. 根调度与提交（概览）
 * - ensureRootIsScheduled：把根加入调度并在微任务末尾统一处理
 * - performSyncWorkOnRoot：同步路径入口
 **********************/
function ensureRootIsScheduled(root) {
  // 把根加入链表，并标记可能有待同步工作
  // 在微任务末尾统一调度各根（act 环境有特殊分支）
  // ...
}
...
/**********************
 * F. flushSync：强制同步刷新（当前行为）
 * - 设置 BatchedContext，提升到 DiscreteEventPriority
 * - 若不在 render/commit 上下文，结尾处会同步 flush 所有根的同步工作
 **********************/
function flushSyncFromReconciler(fnOrVoid) {
  ...
  try {
    ...
    return typeof fnOrVoid === "function" ? fnOrVoid() : undefined;
  } finally {
    setCurrentUpdatePriority(previousPriority);
    ReactSharedInternals.T = prevTransition;
    executionContext = prevExecutionContext;
    if ((executionContext & (RenderContext | CommitContext)) === NoContext) {
      // 不在渲染/提交上下文：同步 flush 所有根的同步工作
...
- 事件系统按事件类型选择优先级：如离散事件会以 `DiscreteEventPriority` 运行。
- `scheduleUpdateOnFiber(root, fiber, lane)` 合并调度至。
- Hook 更新队列使用环形链表，保留“急切状态（eagerState）”优化以减少不必要渲染。
- 根调度器在微任务末尾统一推进工作循环，实现自动批量。
