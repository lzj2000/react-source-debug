...
// 非空闲优先级组合：包含除空闲（Idle）、屏幕外（Offscreen）、延迟（Deferred）外的所有优先级
...
export function getEventPriority(domEventName: DOMEventName): EventPriority {
  switch (domEventName) {
    ...
    // 消息事件：按调度优先级映射
    case "message": {
      const level = getCurrentSchedulerPriorityLevel();
      switch (level) {
...
export function resolveUpdatePriority(): EventPriority {
  ...
  // 无则取当前事件类型对应的优先级
  return window.event
    ? getEventPriority(window.event.type)
...
当组件中调用 `setState` 时，React 会通过 `scheduleUpdateOnFiber` 将更新标记在 Fiber 树上，并最终调用 `ensureRootIsScheduled`，这是连接 Reconciler 和 Scheduler 的关键桥梁。
...
React 的 Lane 优先级模型与时间切片调度是支撑并发渲染的两大核心支柱，二者协同工作，从任务优先级量化与任务执行调度两个维度，解决了传统同步渲染中主线程阻塞、交互响应延迟的痛点，实现了 “可中断、可恢复、优先级驱动” 的现代前端渲染范式。

从技术演进的角度，Lane 与时间切片的组合不仅是 React 自身的重大突破，也为前端异步渲染提供了经典的参考范式。它揭示了现代前端框架的核心设计思路：以用户体验为核心，通过精细化的优先级管理与协作式的任务调度，实现主线程资源的最优分配。
